...
@Client.on_message(filters.command(["همسه"], ""))
...
@Client.on_message(filters.command("start"), group=473)
...
@Client.on_message(filters.private & filters.text & ~filters.command("start"), group=921)
...
@Client.on_callback_query(filters.regex("hms_answer"))
...
@Client.on_callback_query(filters.regex("hms_cancel"))
...
@Client.on_callback_query(filters.regex("hms_confirm"))
...
async def handle_mentions(client, message):
    text = message.text
    if "@username" in text:  # Replace with your bot's username
