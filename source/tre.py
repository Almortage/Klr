...
async def zahrafa(client, message):
    text = message.text.split(None, 1)[1]
    if len(message.text) > 300:
        await message.reply_text("◍ لا يمكنك زخرفه اكثر من 20 حرف ارسل مجددا\n√", reply_to_message_id=message.message_id)
        return
    else:
        if re.match("\n", str(message.text)):
            await message.reply_text("◍ لا يمكن زخرفه نص يحتوي على اكثر من سطر\n√", reply_to_message_id=message.message_id)
            return
    EmojeS = [
        ' 𓁻',
        ' 𓏴  ',
    ...
    Text_Zhrfa = "1- `" + zhrf + randomessage.choice(EmojeS) \
    ...
    await message.reply_text(Text_Zhrfa, reply_to_message_id=message.message_id, parse_mode="Markdown")
