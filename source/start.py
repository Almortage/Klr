...
"مره واحد اسمو المرتجل  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
...
[InlineKeyboardButton(f".♪ 𝑫𝒆𝒗 𝒔𝒐𝒖𝒓𝒄𝒆 ♪.", url=f"https://t.me/{DEV_USER}")],
...
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="A_q_lp")
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["A_q_lp"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["A_q_lp"]:
   ...
   if message.from_user.username == "A_q_lp":
